...
def create_annotation(
    db: Session, annotation: schemas.AnnotationCreate, user_id: uuid.UUID
) -> models.Annotation:
...
def get_annotations_for_article(
    db: Session, article_id: uuid.UUID, skip: int = 0, limit: int = 100
...
def get_or_create_named_entity(
    db: Session, entity: schemas.NamedEntityCreate
) -> models.NamedEntity:
...
def link_article_to_entity(
    db: Session, article_id: uuid.UUID, entity_id: uuid.UUID, count: int = 1
):
    ...
    db_link = models.ArticleEntity(
        article_id=article_id, entity_id=entity_id, count=count
    )
    ...
    try:
        ...
        Exception
    ):  # Catch IntegrityError for existing links, potentially update count
        db.rollback()
